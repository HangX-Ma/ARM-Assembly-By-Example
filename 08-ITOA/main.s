...
.macro      pow     base, exp
    mov     r0, \base
    mov     r1, \exp
    cmp     r1, #0          @ If exponent is 0, the returned value we be 1
    moveq   r2, #1          @ set r0 to 1
    beq     2f
    mov     r2, r0          @ move base number to r2
    sub     r1, #1          @ sub one off expo because we copied base 
1:
    cmp     r1, #0          @ test if we need to leave the loop
...
findstart:
    pow     #10, r7         @ get current power of ten
