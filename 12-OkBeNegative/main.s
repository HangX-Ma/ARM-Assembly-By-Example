...
_start: 
    ldr     r4, =innums     @ load numbers address
    mov     r5, #0          @ init group count 
loop:
    ldrsh   r6, [r4], #2    @ load first number and increment to 
                            @ next address 
    cmp     r6, #0          @ test if eq to null  
    beq     exit            
    ldrsh   r7, [r4], #2    @ load second number
    teq     r6, r7          @ r6 eor r7 
    addpl   r5, #1          @ add if N set 
    b       loop 
exit: 
    @ setup itoa            
    mov     r0, r5          @ copy total to r0 
    ...
    mov     r7, #4          @ 4 = write 
    mov     r0, #1          @ 1 = stdout 
    ldr     r1, =outstr     @ noop - here for clarity 
